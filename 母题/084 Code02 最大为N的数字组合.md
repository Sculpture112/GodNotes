...
> **原题链接:** (https://leetcode.cn/problems/numbers-at-most-n-given-digit-set/)

**涉及知识点:** [[数位DP]], [[组合数]]，[[已AC]],[[预处理]],[[]]
...
public:
    int atMostNGivenDigitSet1(vector<string>& strs, int num) {
        ...
        for (int i = 0; i < (int)strs.size(); i++) digits[i] = stoi(strs[i]);
        //string to int
    ...
    int f1(vector<int>& digits, int num, int offset, int len, int free, int fix) {
        ...
        if (free == 0) {
            // 不能自由选择的情况
            for (int i : digits) {
                if (i < cur) ans += f1(digits, num, offset / 10, len - 1, 1, 1);
                else if (i == cur) ans += f1(digits, num, offset / 10, len - 1, 0, 1);
                else break;
            }
        } else {
            // 可以自由选择的情况
    ...
    int atMostNGivenDigitSet(vector<string>& strs, int num) {
        int m = strs.size();
        vector<int> digits(m);
        for (int i = 0; i < m; i++) digits[i] = stoi(strs[i]);
        int len = 1;
        int offset = 1;
        int tmp = num / 10;
        while (tmp > 0) {
