...
struct FoodCmp {
    bool operator()(const Food& x, const Food& y) const {
        return static_cast<long long>(x.c) * y.b <= static_cast<long long>(y.c) * x.b;
    }
};
...
long long compute() {
    sort(food + 1, food + n + 1, FoodCmp());
    dp[0] = 0;
    for (int i = 1; i <= m; i++) {
        dp[i] = -1;
    }
    for (int i = 1; i <= n; i++) {
        for (int j = m; j >= food[i].c; j--) {
            int p = j - food[i].c;
            if (dp[p] != -1) {
                dp[j] = max(dp[j], dp[p] + food[i].a - static_cast<long long>(j) * food[i].b);
            }
        }
    }
    long long ans = 0;
