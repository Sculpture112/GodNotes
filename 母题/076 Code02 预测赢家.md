...
class Code02_PredictTheWinner {
public:
    // ==========================================
    // 解法一：暴力尝试
    // ==========================================
    static bool predictTheWinner1(vector<int>& nums) {
        int sum = 0;
        for (int num : nums) {
            sum += num;
        }
    ...
    static int f1(const vector<int>& nums, int l, int r) {
        if (l == r) {
            return nums[l];
        }
        if (l == r - 1) {
        ...
        // l....r 不只两个数
        // 可能性1 ：玩家1拿走nums[l] 剩下的由对手在l+1...r中做最优选择，玩家1只能拿对手剩下的最差结果
        int p1 = nums[l] + min(f1(nums, l + 2, r), f1(nums, l + 1, r - 1));
        // 可能性2 ：玩家1拿走nums[r] 剩下的由对手在l...r-1中做最优选择
    ...
    static int f2(const vector<int>& nums, int l, int r, vector<vector<int>>& dp) {
        if (dp[l][r] != -1) {
            return dp[l][r];
        }
        int ans;
