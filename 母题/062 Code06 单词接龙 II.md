...
> **原题链接:** (https://leetcode.cn/problems/word-ladder-ii/)

**涉及知识点:** [[已AC]], [[BFS]]，[[需重刷]],[[DFS]].[[回溯算法]]，[[]]
...
**核心套路:** 
- **BFS 负责“探路与留下路标”**：通过按层扩散（Level-order）找到到目标单词的最短距离，并在沿途构建一棵**反向依赖图（DAG）**。特别注意“延迟删除”机制——必须等当前层全部遍历完，才能从字典中统一抹除这些单词，以此保证同一层的兄弟节点如果指向同一个下一层节点，都能被记录下来。
    
- **DFS 负责“捡芝麻”**：根据 BFS 建立的这棵没有环的纯净反向图，从 `endWord` 开始，利用回溯法逆推回 `beginWord`，拼装出所有合法路径。
**前置母题/相似题目:** [[]]
**时空限制瓶颈:** []
