...
**涉及知识点:** [[已AC]], [[Dijkstra]]，[[需重刷]],[[分层图]],[[DP]],[[链式前向星]]
...
**核心套路:** 
**状态扩展（分层图思想的隐式实现）**。将单纯的最短路状态从“当前所在节点 $u$”扩展为“二维状态 $(u, j)$”，表示“当前在节点 $u$，且已经使用了 $j$ 次免费机会”。对于每条无向边 $(u, v, w)$，存在两种状态转移路线： 1. 正常购票：$(u, j) \to (v, j)$，花费 $w$。 2. 使用免费机会：$(u, j) \to (v, j+1)$，花费 $0$（前提是 $j < k$）。 利用 Dijkstra 算法的贪心性质，第一次从堆中弹出终点状态 $(t, \text{any})$ 时的花费，即为全局最优解。
**前置母题/相似题目:** [[]]
**时空限制瓶颈:** []
