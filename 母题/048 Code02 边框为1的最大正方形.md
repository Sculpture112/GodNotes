...
public:
    ...
    int largest1BorderedSquare(vector<vector<int>>& grid) {
        ...
        if (regionSum(grid, 0, 0, n - 1, m - 1) == 0) {
            return 0;
        }
        ...
        for (int a = 0; a < n; a++) {
            for (int b = 0; b < m; b++) {
                ...
                for (int c = a + ans, d = b + ans, k = ans + 1; c < n && d < m; c++, d++, k++) {
                    if (regionSum(grid, a, b, c, d) - regionSum(grid, a + 1, b + 1, c - 1, d - 1) == (k - 1) << 2) {
                        ans = k;
                    }
                }
            }
...
private:
	...
	//求出区域的和
    int regionSum(const vector<vector<int>>& g, int a, int b, int c, int d) {
