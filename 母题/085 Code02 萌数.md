...
> **原题链接:** (https://www.luogu.com.cn/problem/P3413)
...
// 返回<=num且不是萌数的数字有多少个
int f(const string& num, int i, int pp, int p, int free) {
    ...
    if (free == 0) {
        if (p == 10) {
            ...
            for (int cur = 1; cur < num[i] - '0'; cur++) ans = (ans + f(num, i + 1, p, cur, 1)) % MOD;
            ...
            if (pp != num[i] - '0' && p != num[i] - '0') ans = (ans + f(num, i + 1, p, num[i] - '0', 0)) % MOD;
        }
    } else {
        if (p == 10) {
            // free == 1，能自由选择数字
            ...
            for (int cur = 1; cur <= 9; cur++) ans = (ans + f(num, i + 1, p, cur, 1)) % MOD;
        } else {
            // free == 1，能自由选择数字
            // 之前选择过数字
...
bool check(const string& num) {
    for (int pp = -2, p = -1, i = 0; i < (int)num.size(); pp++, p++, i++) {
