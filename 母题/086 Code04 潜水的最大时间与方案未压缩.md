        ...
        for (int j = 0; j <= m; j++) {
            for (int k = 0; k <= v; k++) {
                ...
                if (j >= a[i] && k >= b[i]) {
                    ...
                    if (path[i - 1][j - a[i]][k - b[i]].empty()) p2 = to_string(i);
                    else p2 = path[i - 1][j - a[i]][k - b[i]] + " " + to_string(i);
                    int value = dp[i - 1][j - a[i]][k - b[i]] + c[i];
                    if (dp[i][j][k] < value) {
                        dp[i][j][k] = value;
                        path[i][j][k] = p2;
                    } else if (dp[i][j][k] == value && p2 < path[i][j][k]) {
...
int main() {
    ...
    while (cin >> m >> v >> n) {
        ...
        dp.assign(n + 1, vector<vector<int>>(m + 1, vector<int>(v + 1, 0)));
        path.assign(n + 1, vector<vector<string>>(m + 1, vector<string>(v + 1)));
        compute();
        cout << dp[n][m][v] << '\n';
        cout << path[n][m][v] << '\n';
    }
